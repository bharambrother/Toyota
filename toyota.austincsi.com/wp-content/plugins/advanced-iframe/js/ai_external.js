 ...
 *  Created: 2016-06-16 14:43:43 
*/ 
if (typeof domain_advanced_iframe === 'undefined') {
    var domain_advanced_iframe = 'http://toyo.mediagate.com/wp-content/plugins/advanced-iframe'; // Check if this is this is the path to the main directory of aip
}
      ...
      if (updateIframeHeight == 'true') {      
        ...
        if (iframe_advanced_iframe_last_height != newHeight) { // we only resize if we have a change
            ...
            if (onload_resize_delay == 0) {
               ...
               if (usePostMessage) {
                 parent.postMessage('type=height&' + send_data,'http://toyo.mediagate.com');
               } else {
                 var iframe = document.getElementById('ai_hidden_iframe_advanced_iframe'); 
                 var url_str = url + '?' + send_data;
                 iframe.src = url_str; 
               }
        ...
        if (usePostMessage) {
           parent.postMessage('type=show&id=' + iframe_id_advanced_iframe,'http://toyo.mediagate.com');
        } else {
          // add the iframe dynamically
...
function resizeLater_advanced_iframe() {
   ...
   if (newHeight > 10) { // Only resize if the height is > 10   
     var send_data = url + '?height=' + (newHeight + 4) + "&width=" + iframeWidth + "&id=" + iframe_id_advanced_iframe;
     if (add_iframe_url_as_param=="remote") {
         send_data += "&loc=" + encodeURIComponent(window.location);
     }
     if (usePostMessage) {
...
if (doIt) {
  ...
  window.jQuery || document.write("<script src='http://toyo.mediagate.com/wp-includes/js/jquery/jquery.js'></script>");
  
  // load the resize script if needed.
  if (resize_on_element_resize != '') {
      document.write("<script src='http://toyo.mediagate.com/wp-content/plugins/advanced-iframe/includes/scripts/jquery.ba-resize.min.js'></script>");
  }
