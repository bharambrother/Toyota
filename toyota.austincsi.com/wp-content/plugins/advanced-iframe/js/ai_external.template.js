      ...
      if (updateIframeHeight == 'true') {      
        ...
        if (iframe_PARAM_ID_last_height != newHeight) { // we only resize if we have a change
            ...
            if (onload_resize_delay == 0) {
               ...
               if (usePostMessage) {
                 parent.postMessage('type=height&' + send_data,'POST_MESSAGE_DOMAIN');
               } else {
        ...
        if (usePostMessage) {
           parent.postMessage('type=show&id=' + iframe_id_PARAM_ID,'POST_MESSAGE_DOMAIN');
        } else {
          // add the iframe dynamically
...
function resizeLater_PARAM_ID() {
   ...
   if (newHeight > 10) { // Only resize if the height is > 10   
     var send_data = url + '?height=' + (newHeight + 4) + "&width=" + iframeWidth + "&id=" + iframe_id_PARAM_ID;
     if (add_iframe_url_as_param=="remote") {
         send_data += "&loc=" + encodeURIComponent(window.location);
     }
     if (usePostMessage) {
